...
describe('EZHostSDK', () => {
  ...
  describe('shortenUrl', () => {
    ...
    it('should handle API errors', async () => {
      ...
      await expect(sdk.shortenUrl(testUrl)).rejects.toThrow(
        `Failed to shorten URL: ${errorMessage}`
      );
    });
    ...
    it('should handle network errors', async () => {
      ...
      await expect(sdk.shortenUrl(testUrl)).rejects.toThrow('Failed to shorten URL: Network Error');
    });

    it('should throw error if URL exceeds maximum length', async () => {
      const longUrl = 'https://example.com/' + 'a'.repeat(2048);
      await expect(sdk.shortenUrl(longUrl)).rejects.toThrow(
    ...
    it('should handle custom maxUrlLength option', async () => {
      ...
      await expect(sdk.shortenUrl(longUrl, { maxUrlLength: 50 })).rejects.toThrow(
        'URL exceeds maximum length of 50 characters'
      );
    });
    ...
    it('should handle non-Axios errors', async () => {
      ...
      await expect(sdk.shortenUrl(testUrl)).rejects.toThrow('Custom error');
    });
    ...
    it('should pass through non-Axios errors without wrapping them', async () => {
      ...
      await expect(sdk.shortenUrl(testUrl)).rejects.toBe(customError);
    });
    ...
    it('should handle Axios errors with no response data', async () => {
      ...
      await expect(sdk.shortenUrl(testUrl)).rejects.toThrow('Failed to shorten URL: ');
    });
    ...
    it('should handle Axios error with no data or message', async () => {
      ...
      await expect(sdk.shortenUrl(testUrl)).rejects.toThrow('Failed to shorten URL: undefined');
    });
    ...
    it('should use error.message when response.data.message is empty', async () => {
      ...
      await expect(sdk.shortenUrl(testUrl)).rejects.toThrow(
        'Failed to shorten URL: Network timeout'
      );
    });
    ...
    it('should handle Axios error with empty response and message', async () => {
      ...
      await expect(sdk.shortenUrl('https://example.com')).rejects.toThrow(
    ...
    it('should handle custom errors directly from the shortener module', async () => {
      ...
      await expect(sdk.shortenUrl('https://example.com')).rejects.toBe(customError);
    });
  });
  ...
  describe('uploadFile', () => {
    ...
    it('should throw error if file is not provided', async () => {
      await expect(sdk.uploadFile(null as unknown as Buffer)).rejects.toThrow('File is required');
    });
    ...
    it('should handle file upload API errors', async () => {
      ...
      await expect(sdk.uploadFile(file)).rejects.toThrow(`Failed to upload file: ${errorMessage}`);
    });
  });
  ...
  describe('createPaste', () => {
    ...
    it('should throw error if text is not provided', async () => {
      await expect(sdk.createPaste('')).rejects.toThrow(
        'Paste text is required and cannot be empty'
      );
    });
    ...
    it('should handle paste creation API errors', async () => {
      ...
      await expect(sdk.createPaste(testText)).rejects.toThrow(
        `Failed to create paste: ${errorMessage}`
      );
    });
  });
});

...
describe('EZHostSDK Upload', () => {
  ...
  describe('uploadFile', () => {
    ...
    it('should reject text files', async () => {
      ...
      await expect(sdk.uploadFile(file, 'text.txt')).rejects.toThrow(
    ...
    it('should handle API errors', async () => {
      ...
      await expect(sdk.uploadFile(file, 'image.jpg')).rejects.toThrow(
    ...
    if (typeof window !== 'undefined') {
      ...
      it('should reject Blob uploads with text MIME types', async () => {
        ...
        await expect(sdk.uploadFile(blob, 'text.txt')).rejects.toThrow(
          'Text files should be uploaded using the createPaste method instead of uploadFile'
        );
      });
    ...
    it('should handle unsupported MIME types', async () => {
      ...
      await expect(sdk.uploadFile(file)).rejects.toThrow(
    ...
    it('should handle non-Axios errors', async () => {
      ...
      await expect(sdk.uploadFile(file, 'test.pdf')).rejects.toBe(customError);
    });
    ...
    it('should handle Axios error with no response data', async () => {
      ...
      await expect(sdk.uploadFile(file, 'test.pdf')).rejects.toThrow(
    ...
    it('should handle Axios error with empty response', async () => {
      ...
      await expect(sdk.uploadFile(file, 'test.pdf')).rejects.toThrow('Failed to upload file: ');
    });
    ...
    it('should handle Buffer with failing Blob creation', async () => {
      ...
      global.Blob = class MockBlob extends originalBlob {
        constructor(...args: [BlobPart[], BlobPropertyBag?]) {
          blobCallCount++;
          if (blobCallCount <= 2) {
  ...
  it('should handle alternative Buffer conversion when primary fails', async () => {
    ...
    try {
      ...
      expect(MockBlob.callCount).toBe(2);
    } finally {
      global.Blob = originalBlob;
    }
  });
